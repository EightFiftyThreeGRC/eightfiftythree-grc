...
| 2 | Role & access matrix | **Sonnet** | Procedural with judgment | Impersonate each role via the sidebar role picker; assert visible tabs == `ROLE_TABS`; SoD: issue verifier ≠ assignee, risk acceptance limited to program owner/AO; AO decision modal gating (`atoCanDecide`) |
