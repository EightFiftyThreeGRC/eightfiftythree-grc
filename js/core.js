...
const state = {
  ...
  // Organization profile (retired 2026-08-26). Leftover keys on saved programs are
  // ignored by setup \u2014 overlays and the common-control floor are not derived from them.
  orgSizeBand: '',          // leftover: workforce size
  orgDataTypes: [],         // leftover: regulated / sensitive data in scope
  orgImpactProfile: '',     // leftover: FIPS 199 style worst-case consequence
  orgNonUsFootprint: '',    // leftover: non-US footprint (was ISO 27001 signal)
  orgSoc2Demand: '',        // leftover: SOC 2 demand signal
  ...
  policyMapWizardRev: 0,     // 2+ = Profile once inserted into Path B; 3 = unified wizard; 4 = Profile step removed (7-step Path A)
  ...
  policyFamilyHome: {},          // sparse { 'SC': 'DE' or 'standalone' } override of CSF family-to-Function home
  ...
  _csfProfileDetail: false,      // CSF Profile panel: show per-outcome rows
  _customRegLibraryId: null,     // Framework alignment: open custom-reg requirement library
  _landIspReviewAfterRoleSwitch: false, // route to ISP review after role switch
  _cisoOwnerEditFam: null,       // Assign Owners: row currently in edit mode
  _frameworkFilter: '',
  _frameworkSearch: '',
...
// Precedence is deliberate: larsen (the newer brand) is migrated first; if both
// prefixes hold data, the older hawthorn copy is superseded and removed unread.
...
function migrateCisoWizardDropProfileStep() {
  ...
  if (rev >= 4) return;
  var s = parseInt(state.cisoSetupStep, 10) || 1;
  var max = (typeof CISO_WIZARD_STEPS === 'number') ? CISO_WIZARD_STEPS : 7;
  if (s >= 3) s = s - 1;
  if (s < 1) s = 1;
  if (s > max) s = max;
  state.cisoSetupStep = s;
  if (typeof currentStep !== 'undefined') currentStep.ciso = s;
  state.policyMapWizardRev = 4;
}
...
function applyLoadedState(saved) {
  ...
  // Legacy Phase-1 POA&M payloads: poamItems is no longer a state key, so carry it
  // across explicitly for migratePoamItemsToIssues() (which deletes it when done).
  if (Array.isArray(saved.poamItems) && saved.poamItems.length) state.poamItems = saved.poamItems;
  normalizeStateShape();
...
function importProgramFromFile(ev) {
  ...
  reader.onload = function() {
    try {
      ...
      // Reset first so keys absent from the imported file don't bleed through
      // (mirrors applySnapshotFromDataString in js/app.js).
